// Update all user passwords to "password" on PRODUCTION database
const mongoose = require('mongoose');
const bcrypt = require('bcryptjs');

// PRODUCTION MongoDB URI (same as in your production environment)
...
async function updateProductionPasswords() {
  try {
    ...
    console.log('🔐 Updating all PRODUCTION student passwords...');
    const studentResult = await Student.updateMany(
      {}, // Update all students
      { $set: { password: hashedPassword } }
    );
    console.log(`✅ Updated ${studentResult.modifiedCount} student passwords in PRODUCTION`);
    
    console.log('🔐 Updating all PRODUCTION employee passwords...');
    const employeeResult = await Employee.updateMany(
      {}, // Update all employees
      { $set: { password: hashedPassword } }
    );
    console.log(`✅ Updated ${employeeResult.modifiedCount} employee passwords in PRODUCTION`);
    
    // Show some sample users for testing
    console.log('\n📋 PRODUCTION login credentials (all passwords are "password"):');
    ...
    console.log('\n🔑 All PRODUCTION users now have password: "password"');
    console.log('🌐 You can login at: https://dmudms-next.vercel.app/auth/signin');
    console.log('\n🎯 Same credentials work on both:');
    console.log('   • Local: http://localhost:3000/auth/signin');
    console.log('   • Production: https://dmudms-next.vercel.app/auth/signin');
    
    await mongoose.disconnect();
    console.log('✅ Disconnected from PRODUCTION MongoDB');
    
  } catch (error) {
    console.error('❌ Error updating PRODUCTION passwords:', error);
  }
}
